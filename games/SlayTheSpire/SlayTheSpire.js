...
function shuffle() {
  ...
  var currentIndex = drawPile.length,
...
function play() {
  if (selectedCard != null) {
    ...
    if (monster.HP <= 0) {
      alert("Monster died! Victory :)")
    } else {
      turn++
    }
    if (player.HP <= 0) {
      alert("You died! Defeat :(")
    } else {
      turn++
    }
  ...
  for (var i = deck.length - 1; i >= 0; i--) {
    ...
    /*
    	...
    	switch (type){
    		...
    		case "defend":
    			cardBlock = 5;
        		cardText.innerHTML = "Gain " + (cardBlock/* + player.dexterity*/
    /*) + " block.";
        		cardTitle.innerHTML = "Defend";
        		cardImage.setAttribute("src", "defend.png");
        		cardImage.setAttribute("alt", "Defend");
